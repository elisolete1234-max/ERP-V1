...
export async function GET(_request: Request, context: RouteContext) {
  try {
    ...
    doc.roundedRect(doc.x, doc.y, pageWidth, 96, 16).fillAndStroke("#eff6ff", "#bfdbfe");
    doc.font(fontPath).fillColor(ink).fontSize(24).text("Eli Print 3D", 72, 70);
    doc.font(fontPath).fillColor(accent).fontSize(15).text("Produccion 3D profesional", 72, 98);
    doc.font(fontPath).fillColor(ink).fontSize(13).text(invoice.codigo, 72, 122);
    doc
      .font(fontPath)
      .fillColor(muted)
      .fontSize(10)
      .text(`Fecha factura: ${formatDate(invoice.fecha)}`, 72, 146)
      .text(`Estado de pago: ${invoice.resumen.estadoPago.toLowerCase()}`, 260, 146);
    ...
    const drawBlock = (title: string, rows: Array<[string, string]>, x: number, width: number) => {
      ...
      doc.font(fontPath).fillColor(ink).fontSize(12).text(title, x + 16, startY + 14);
      let rowY = startY + 40;
      rows.forEach(([label, value]) => {
        doc.font(fontPath).fillColor(muted).fontSize(9).text(label, x + 16, rowY);
        doc.font(fontPath).fillColor(ink).fontSize(10).text(value || "-", x + 16, rowY + 12, {
          width: width - 32,
        });
        rowY += 34;
      });
    ...
    doc.y += 138;
    doc.font(fontPath).fillColor(ink).fontSize(12).text("Productos facturados");
    doc.y += 12;
    ...
    doc.font(fontPath).fillColor("white").fontSize(9);
    ...
    invoice.lineas.forEach((linea: (typeof invoice.lineas)[number]) => {
      ...
      doc.font(fontPath).fillColor(ink).fontSize(10).text(
        `${linea.producto_codigo} - ${linea.producto_nombre}`,
        columns.producto + 12,
        doc.y + 8,
        { width: 230 },
      );
      doc.font(fontPath).fillColor(muted).fontSize(8).text(linea.codigo, columns.producto + 12, doc.y + 22);
      doc.font(fontPath).fillColor(ink).fontSize(10);
    ...
    ensureSpace(doc, 220);
    doc.y += 8;
    doc.font(fontPath).fillColor(ink).fontSize(12).text("Resumen economico");
    doc.y += 10;
    ...
    summaryRows.forEach(([label, value], index) => {
      ...
      doc.font(fontPath).fillColor(muted).fontSize(9).text(label, summaryX + 16, summaryY);
      doc.font(fontPath).fillColor(ink).fontSize(index >= 4 ? 11 : 10).text(value, summaryX + 140, summaryY, {
    ...
    doc.font(fontPath).fillColor(accent).fontSize(10).text(
      `Estado de pago: ${invoice.resumen.estadoPago}`,
      summaryX + 16,
      doc.y + 146,
    );

    doc.font(fontPath).fillColor(ink).fontSize(12).text("Pagos registrados", doc.page.margins.left, doc.y);
    doc.y += 10;

    if (invoice.pagos.length === 0) {
      doc.roundedRect(doc.page.margins.left, doc.y, 250, 54, 12).fillAndStroke(soft, line);
      doc.font(fontPath).fillColor(muted).fontSize(10).text(
      ...
      invoice.pagos.forEach((payment: (typeof invoice.pagos)[number]) => {
        ...
        doc.font(fontPath).fillColor(muted).fontSize(9).text(
    ...
    doc.font(fontPath).fontSize(9).fillColor(muted).text(
