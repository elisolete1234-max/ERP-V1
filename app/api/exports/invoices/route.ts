...
export async function GET(request: Request) {
  ...
  const csv = serializeCsv(rows, {
    columns: [
      ...
      { header: "subtotal_iva_incluido", value: (row) => formatCsvMoney(row.subtotal) },
      { header: "descuento_iva_incluido", value: (row) => formatCsvMoney(row.descuento) },
      { header: "base_imponible", value: (row) => formatCsvMoney(row.baseImponible) },
      { header: "iva_incluido", value: (row) => formatCsvMoney(row.iva) },
